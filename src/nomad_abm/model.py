...
The core nomad-household agent-based model for the Negev Highlands
Iron Age IIA simulation. This is the canonical location for the model
code; ``Code/model_opt.py`` is a backwards-compatibility shim that
re-exports the symbols defined here.
"""
...
class LazyYearlyData:
    """
    Optimized Lazy loader for yearly HDF5 data.
    Keeps the file handle open within a process to reduce I/O overhead,
    while remaining pickle-safe for multiprocessing.
    """

    def __init__(self, filepath):
        self.filepath = filepath
        self._file = None  # Cached file handle

        if os.path.exists(filepath):
            # Open briefly to read metadata
    ...
    def __getstate__(self):
        ...
        We exclude the file handle '_file' because opened files cannot be pickled.
        """
    ...
    def __setstate__(self, state):
        """
        Called when the object is unpickled (received by a worker).
        We restore the state and ensure '_file' is None so it gets opened lazily.
        """
    ...
    @property
    def file(self):
        """Lazy accessor that opens the file only if needed and keeps it open."""
        if self._file is None or not self._file.id.valid:
            self._file = h5py.File(self.filepath, 'r')
        return self._file

    def __getitem__(self, idx):
        # Use the cached file handle via self.file
        group = self.file[f'group_{idx}']

        # Assuming keys are array_0, array_1...
        keys = sorted(group.keys(), key=lambda x: int(x.split('_')[1]))
        arrays = [np.array(group[k]) for k in keys]
        return arrays

    def close(self):
        """Explicitly close the file if needed."""
        if self._file is not None:
            self._file.close()
            self._file = None

def load_permanent_data():
    """Loads permanent results similar to the notebook logic."""
    ...
    with h5py.File(PERMANENT_DATA_PATH, 'r') as f:
        ...
        # Notebook logic iterates and overwrites, effectively keeping the last group
...
class SharedDataManager:
    """
    Manages shared memory for large numpy arrays to prevent duplication
    ...
    def create_shared_block(name, array):
        """Called by Main Process: Allocates shared memory and copies data."""
        # Clean up if exists
        try:
        ...
        shared_array[:] = array[:]  # Copy data
        return shm  # Keep reference to prevent garbage collection

    @staticmethod
    def get_array(name, shape, dtype):
        """Called by Worker: Connects to existing shared memory."""
        if name not in SharedDataManager._arrays:
            try:
                shm = shared_memory.SharedMemory(name=name)
                SharedDataManager._shm_cache[name] = shm
                # Create numpy view (no data copy)
                SharedDataManager._arrays[name] = np.ndarray(shape, dtype=dtype, buffer=shm.buf)
            except FileNotFoundError:
                raise RuntimeError(f"Shared memory block '{name}' not found. Did you initialize it in the notebook?")
...
def ensure_data_loaded():
    """
    Called inside the worker process. Connects to shared memory.
    """
    ...
    try:
        ...
        # Load permanent results (assuming they are named perm_res_0, perm_res_1, etc.)
        ...
        # Fallback for non-shared memory execution (e.g. single process debugging)
    ...
    # Initialize Lazy Loader for yearly data if not present
...
def env_degrade(model, mesa_x, mesa_y, r, d_factor, p_factor):
    """Optimized environment degradation using vectorized kernels."""
...
def Yi_params(year, accumulator, latest_output):
    """Optimized Yi_params using incremental updates."""
    if year > 0:
        # Incremental update: New = Old * 0.5 + Current_Term
        term = latest_output.copy()
        # Apply weights to current term (1.0 if < 2, 1.5 if >= 2)
        mask2 = term >= 2
        term[mask2] *= 1.5
...
def get_suitability_raster(y_output, indices, year, weights, accumulator=None, latest_output=None):
    # Unpack permanent results (loaded globally)
    ...
    # Get yearly data using LazyLoader
    random_year = [arr.copy() for arr in y_output[indices[year]]]
...
class NomadModel(mesa.Model):
    def __init__(self, suitability_raster, return_raster, stress_ras, inds, place_raster=None, ras_w=None,
        ...
        for i in range(self.num_agents):
            ...
            env_degrade(self, mesa_x, mesa_y, 25, 0.5, 0.9)
            agent.memory.append([agent.pos, env_mean_val(self, agent.pos, 20), self.year])
    ...
    def move_year(self, inds):
        self.reset_pos()
        # Use self.return_raster as the accumulator for the next year
        ...
        current_year_data = self.y_output[inds[self.year]]
...
def viz_map(model, suitability_raster, year, run_dir):
    """Thread-safe visualization using Figure object directly."""
...
def obj_func(gdf, gdf1, run_dir):
    ...
    if len(points1) == 0: return 1.0, 1.0  # Max error
...
def objective_function(permanent_weights, yearly_weights, trial_n, trial, n_iterations=10, n_years=75,
    ...
    if os.path.exists(CALIB_SHP_PATH):
        gdf_real = gpd.read_file(CALIB_SHP_PATH)
    else:
        # Fallback or error
        return 1.0
    ...
    for i in range(n_iterations):
        current_seed = (random_seed * 100) + i
        pre_w = {**permanent_weights, **yearly_weights}
        w = list(pre_w.values())
        if sum(w) != 1:
            w = np.array(w) / sum(w)

        sim, run_dir = run_model_opt(n_years,
                                     ras_wts=w,
        ...
        cumulative_mean = np.mean(scores)
        trial.report(cumulative_mean, step=i)
...
def objective(trial, sim_y=75, n_iter=10):
    """
    Optuna objective function.
    Handles GPU assignment and parameter sampling.
    """
    ensure_data_loaded()
    # GPU Assignment based on trial number
    gpu_id = trial.number % 3
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    #print(f"DEBUG: Trial {trial.number} (PID {os.getpid()}) assigned to GPU {gpu_id}")
    ...
    sum_w = sum(params.values())
    if sum_w == 0:
        return 9999

    norm_params = {k: v / sum_w for k, v in params.items()}
